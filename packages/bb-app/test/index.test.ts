...
describe("bb-app launcher", () => {
  ...
  it("resolves production defaults for npx startup", () => {
    ...
    expect(context.envFile).toBe(join(join("/home/tester", ".bb"), "env.json"));
  ...
  it("spawns the bundled CLI through the node runtime on Windows", () => {
    ...
    try {
      ...
      expect(
        resolveBundledCliSpawnPlan(join(bundleDir, "missing.cmd"), "win32"),
      ).toEqual({
        argsPrefix: [],
  ...
  it("requires the Windows launcher and its target on Windows hosts", () => {
    ...
    try {
      ...
      expect(() => assertBbHostArtifacts(context, "win32")).toThrow(/bb\.cmd/);
