...
describe("isPathWithinDirectory with win32 paths", () => {
  ...
  it("accepts forward-slash spellings of the same windows paths", () => {
    expect(
      isPathWithinDirectory("C:/plug", "C:/plug/src/server.ts", "win32"),
    ).toBe(true);
  });
  ...
  it("rejects a sibling whose prefix matches but is not contained", () => {
    expect(
      isPathWithinDirectory("C:\\plug", "C:\\plugin-malo\\x.ts", "win32"),
    ).toBe(false);
  });

  it("treats windows paths as case-insensitive", () => {
    expect(
      isPathWithinDirectory("C:\\Plug", "c:\\plug\\src\\x.ts", "win32"),
    ).toBe(true);
  });

  it("rejects entries on another drive", () => {
    expect(isPathWithinDirectory("C:\\plug", "D:\\x.ts", "win32")).toBe(false);
  });
  ...
  it("allows a file segment that starts with dots without escaping", () => {
    expect(isPathWithinDirectory("C:\\p", "C:\\p\\..foo\\x", "win32")).toBe(
      true,
    );
  });
});
...
describe("isPathWithinDirectory with posix paths", () => {
  it("keeps a nested entry inside the plugin", () => {
    expect(
      isPathWithinDirectory("/a/plugin", "/a/plugin/src/server.ts", "linux"),
    ).toBe(true);
  });

  it("rejects a sibling whose prefix matches but is not contained", () => {
    expect(isPathWithinDirectory("/plug", "/plugin-malo/x.ts", "linux")).toBe(
      false,
    );
  });
...
describe("resolveManifestPath across host path formats", () => {
  ...
  it("rejects a windows traversal and an absolute windows entry", () => {
    expect(() =>
      resolveManifestPath("D:\\a\\plugin", "../evil.ts", "bb.server"),
    ).toThrow(/escapes the plugin directory/);
    expect(() =>
      resolveManifestPath("D:\\a\\plugin", "C:\\abs.ts", "bb.server"),
