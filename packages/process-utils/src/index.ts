...
export function killProcessGroup(args: KillProcessGroupArgs): void {
  ...
  if (platform === "win32") {
    if (
      ...
      killWindowsProcessTreeSync(args.child.pid, args.runWindowsTaskkillSync)
    ) {
      return;
    }
    try {
...
async function killWindowsProcessTreeBestEffort(args: {
  ...
  if (pid !== undefined) {
    try {
      ...
      treeHandled =
        result.exitCode === 0 ||
        (isWindowsProcessNotFoundOutput(`${result.stdout}\n${result.stderr}`) &&
          !args.isAlive(pid));
    } catch {
...
function canonicalizeWindowsPath(value: string): string {
  ...
  rest = rest.replace(/\\+/g, "\\");
  if (rest.length > 1 && rest.endsWith("\\") && !/^[A-Za-z]:\\$/.test(rest)) {
    rest = rest.replace(/\\+$/, "");
  }
  if (rest === "") {
...
function isWindowsAbsolutePathCandidate(token: string): boolean {
  return WINDOWS_ABSOLUTE_PATH_PATTERN.test(token) || token.startsWith("\\\\");
}
...
export function matchWindowsProcessesUnderDirectory(
  ...
  const pathCandidates = args.trackedRootsOnly ? [] : args.snapshot;
  ...
  while (queue.length > 0) {
    ...
    for (const childPid of childrenByParent.get(pid) ?? []) {
      ...
      const own =
        child === undefined || args.trackedRootsOnly
          ? null
...
async function listWindowsProcessesWithCwdUnder(args: {
  ...
  return matchWindowsProcessesUnderDirectory({
    ...
    trackedRootsOnly: args.trackedRootsOnly,
  });
}
