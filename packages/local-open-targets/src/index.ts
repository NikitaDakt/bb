...
function isWindowsExplorerInvocation(invocation: ExecFileInvocation): boolean {
...
function getWindowsInstallRoots(runtime: WorkspaceOpenTargetRuntime): string[] {
  ...
  for (const variableName of WINDOWS_INSTALL_ROOT_ENV_VARIABLES) {
    ...
    if (root !== undefined && root.length > 0 && !roots.includes(root)) {
      roots.push(root);
    }
  }
  return roots;
...
function getWindowsCliOpenCommand(
  ...
  return adapter.macos.pathOpenCommand ?? adapter.macos.lineOpenCommand ?? null;
}
...
async function isWindowsCliTargetAvailable(
  ...
  return (
    command !== null &&
    (await resolveWindowsCommandExecutable(adapter, command, runtime)) !== null
  );
}
