  ...
  describe("windows", () => {
    ...
    it("discovers fallback executables on Windows PATH", async () => {
      ...
      try {
        ...
        expect(targets.map((target) => target.id)).toContain("devin-desktop");
    ...
    it("opens files in VS Code at line and column through code.cmd", async () => {
      ...
      try {
        ...
        expect(calls.find((call) => call.file === "where")?.args).toEqual([
          "code",
        ]);
        expect(calls.some((call) => call.file === "which")).toBe(false);
      } finally {
    ...
    it("leaves JetBrains targets unavailable without Toolbox or installs", async () => {
      const targets = await listWorkspaceOpenTargetsWithRuntime(
        createWindowsRuntime({
          env: {
            LOCALAPPDATA: await mkdtemp(path.join(tmpdir(), "bb-empty-")),
          },
        }),
      );
