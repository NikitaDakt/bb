...
if (${JSON.stringify(name)} === "better-sqlite3") {
...
export function ensureNativeModules({
  ...
  verifyNativeModule: verifyNativeModuleImpl = (
    name,
    _requireModule,
    pkgJsonPath,
  ) => {
    const error = getRepairedNativeModuleError(name, pkgJsonPath);
  ...
  for (const {
    ...
    try {
      verifyNativeModuleImpl(name, requireModule, pkgJsonPath);
    } catch (err) {
