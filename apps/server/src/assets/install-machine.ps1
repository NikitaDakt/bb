...
$script:InstallerSource = $MyInvocation.MyCommand.ScriptBlock.ToString()
...
function Write-Usage {
  [Console]::Error.WriteLine('Usage: install.ps1 -BootstrapEnv <NAME> [-HostDaemonPort <port>]')
  [Console]::Error.WriteLine('       install.ps1 -Adopt -DataDir <path> [-HostDaemonPort <port>]')
  [Console]::Error.WriteLine('       install.ps1 <-Start|-Stop|-Restart|-Uninstall> -DataDir <path>')
  [Console]::Error.WriteLine('')
  [Console]::Error.WriteLine('Get the enrollment command from Settings -> Machines or bb machine create --provider manual.')
...
function Resolve-NpmCli {
  if ($script:NpmCli -ne '') {
    return $script:NpmCli
  }
  $npmCmd = Get-Command 'npm.cmd' -ErrorAction SilentlyContinue | Select-Object -First 1
  if ($null -eq $npmCmd) {
    Exit-Fail 'bb-app installation requires npm.'
  }
  $script:NpmCli = Join-Path (Split-Path -Parent $npmCmd.Source) 'node_modules\npm\bin\npm-cli.js'
  if (-not (Test-Path -LiteralPath $script:NpmCli)) {
    Exit-Fail 'Could not find npm-cli.js beside npm.cmd. Reinstall Node.js with npm, then retry.'
  }
  return $script:NpmCli
}
...
function Get-LauncherArguments {
  param([string]$WrapperScript, [string]$LogPath)
  $command = '& ' + (Quote-PowerShell $WrapperScript) + ' *>> ' + (Quote-PowerShell $LogPath)
...
function Start-DetachedDaemonProcess {
  param([string]$WrapperScript, [string]$LogPath, [string]$WorkingDir)
  $powershellExe = Join-Path $env:SystemRoot 'System32\WindowsPowerShell\v1.0\powershell.exe'
  $cmdLine = '"' + $powershellExe + '" ' + (Get-LauncherArguments -WrapperScript $WrapperScript -LogPath $LogPath)
  try {
...
function Install-ScheduledTaskPersistence {
  ...
  $taskCommand = Get-LauncherArguments -WrapperScript $WrapperScript -LogPath $taskLog
...
function Install-RunKeyPersistence {
  ...
  $runKey = 'HKCU:\Software\Microsoft\Windows\CurrentVersion\Run'
  $powershellExe = Join-Path $env:SystemRoot 'System32\WindowsPowerShell\v1.0\powershell.exe'
  $runCommand = '"' + $powershellExe + '" ' + (Get-LauncherArguments -WrapperScript $WrapperScript -LogPath (Join-Path $DataDir 'logs\host-daemon.log'))
  New-ItemProperty -Path $runKey -Name $ValueName -Value $runCommand -PropertyType String -Force -ErrorAction Stop | Out-Null
  if (Test-DaemonStatusMatches -Port $Port -ExpectedHostId $script:HostIdValue -ExpectedServerUrl $ServerUrl -RequireConnected $false) {
  ...
  $daemonPid = Start-DetachedDaemonProcess -WrapperScript $WrapperScript -LogPath $daemonLog -WorkingDir $DataDir
  ...
  Write-Detail ("& " + (Quote-PowerShell $WrapperScript))
...
function Invoke-ServiceLifecycle {
  ...
  if (-not (Test-Path -LiteralPath $wrapper) -or -not (Test-Path -LiteralPath $portFile)) {
    Exit-Fail 'No installed Windows machine service was found in this data directory.'
  }
  $port = [IO.File]::ReadAllText($portFile).Trim()
  if (-not (Test-ValidPort $port)) { Exit-Fail 'The installed host-daemon port is invalid.' }
  if ($Stop -or $Restart -or $Uninstall -or $StopOnly) {
    $listener = Get-ListenerProcessId $port
    if ($listener -ne 0) {
      $identity = Invoke-NodeScript 'void (async () => { const r = await fetch(`http://127.0.0.1:${process.argv[2]}/status`, {signal: AbortSignal.timeout(1500)}); const s = await r.json(); process.exit(r.ok && s.hostId === process.argv[3] ? 0 : 1); })().catch(() => process.exit(1));' @($port, $MachineId)
      if ($identity.ExitCode -ne 0) { Exit-Fail 'The service port belongs to another process; it was left running.' }
      $prefix = Join-Path $Directory 'npm\node_modules\bb-app\'
      $root = $null
      $next = $listener
      for ($depth = 0; $depth -lt 16 -and $next -gt 0; $depth++) {
        $candidate = Get-CimInstance Win32_Process -Filter "ProcessId=$next" -ErrorAction Stop
        ...
        if ($command.IndexOf($prefix, [StringComparison]::OrdinalIgnoreCase) -lt 0 -and $command.IndexOf($wrapper, [StringComparison]::OrdinalIgnoreCase) -lt 0) { break }
        $root = $candidate
        $next = [int]$candidate.ParentProcessId
      }
      if ($null -eq $root) { Exit-Fail 'The listener is not the installed bb runtime; it was left running.' }
      $current = Get-CimInstance Win32_Process -Filter "ProcessId=$($root.ProcessId)" -ErrorAction Stop
      if ($null -ne $current -and $current.CreationDate -eq $root.CreationDate) {
        Stop-DaemonProcess ([int]$root.ProcessId)
      }
    }
    if (Get-ScheduledTaskExists $task) {
      Invoke-NativeCommand 'schtasks' @('/End', '/TN', $task) -MergeErrors $true | Out-Null
    }
  }
  if ($Uninstall) {
    if (Get-ScheduledTaskExists $task) {
      $deleted = Invoke-NativeCommand 'schtasks' @('/Delete', '/TN', $task, '/F') -MergeErrors $true
      if ($deleted.ExitCode -ne 0) { Exit-Fail 'Could not remove the scheduled task.' @($deleted.Output) }
    }
    Remove-RunKeyValue $task
    Remove-Item -LiteralPath $wrapper -Force
    $script:PortRegistryDir = Join-Path (Join-Path $HOME '.bb-machines') 'host-daemon-ports'
    Release-PortForDataDir -Port $port -DataDir $Directory
    Step-Complete 'Removed the machine service; its data and credentials were preserved.'
    return
  }
  if ($Start -or $Restart) {
    if (-not (Test-PortAvailable $port)) {
      if (Test-DaemonStatusMatches $port $MachineId $ServerUrl $false) { return }
      Exit-Fail 'The service port is occupied; its process was left running.'
    }
    if (Get-ScheduledTaskExists $task) {
      $started = Invoke-NativeCommand 'schtasks' @('/Run', '/TN', $task) -MergeErrors $true
      if ($started.ExitCode -ne 0) { Exit-Fail 'Could not start the scheduled task.' @($started.Output) }
    } else {
      $startedPid = Start-DetachedDaemonProcess $wrapper (Join-Path $Directory 'logs\host-daemon.log') $Directory
      [IO.File]::WriteAllText((Join-Path $Directory 'install-daemon.pid'), "$startedPid", (New-Object Text.UTF8Encoding $false))
    }
    if (-not (Wait-DaemonConnection 'the host daemon' $port $MachineId $ServerUrl)) { Exit-Fail 'The machine service did not reconnect.' }
  }
  Step-Complete 'Machine service updated'
}
...
if (@(@($Adopt, $Start, $Stop, $Restart, $Uninstall) | Where-Object { $_ }).Count -gt 1) { Exit-Usage }
$existingIdentity = $Adopt -or $Start -or $Stop -or $Restart -or $Uninstall
$bootstrapPayload = $null
if ($existingIdentity) {
  if ($BootstrapEnv -ne '') { Exit-Usage }
  if ([string]::IsNullOrWhiteSpace($requestedDataDir)) { $requestedDataDir = $env:BB_DATA_DIR }
  if ([string]::IsNullOrWhiteSpace($requestedDataDir)) { Exit-Usage }
  try {
    $requestedDataDir = [IO.Path]::GetFullPath($requestedDataDir)
    $auth = ConvertFrom-Json ([IO.File]::ReadAllText((Join-Path $requestedDataDir 'auth.json')))
    $config = ConvertFrom-Json ([IO.File]::ReadAllText((Join-Path $requestedDataDir 'config.json')))
    $headers = [pscustomobject]@{}
    if ($config.PSObject.Properties['serverHeaders'] -and $null -ne $config.serverHeaders) { $headers = $config.serverHeaders }
    elseif ($config.PSObject.Properties['machineCredential']) { $headers = [pscustomobject]@{ 'x-bb-connect-machine' = $config.machineCredential } }
    $bootstrap = [pscustomobject]@{ hostId = $auth.hostId; serverUrl = $config.serverUrl; dataDir = $requestedDataDir; headers = $headers }
  } catch { Exit-Fail 'This data directory has no valid enrolled machine identity and server configuration.' }
} else {
  if ($BootstrapEnv -notmatch '^[A-Za-z_][A-Za-z0-9_]*$') { Exit-Usage }
  $bootstrapPayload = [Environment]::GetEnvironmentVariable($BootstrapEnv)
  [Environment]::SetEnvironmentVariable($BootstrapEnv, $null)
  if ([string]::IsNullOrWhiteSpace($bootstrapPayload)) { Exit-Usage }
  try { $bootstrap = ConvertFrom-Json -InputObject $bootstrapPayload } catch { Exit-Fail 'Invalid enrollment JSON.' }
}
try {
  if ($bootstrap.hostId -notmatch '^[A-Za-z0-9_-]+$') { throw 'Invalid machine ID' }
  if (-not $existingIdentity) {
    if ([string]::IsNullOrWhiteSpace($bootstrap.credential)) { throw 'Invalid enrollment' }
    if ([double]$bootstrap.expiresAt -le [DateTimeOffset]::UtcNow.ToUnixTimeMilliseconds()) { throw 'Enrollment has expired' }
  }
  $serverUri = [Uri]$bootstrap.serverUrl
  if (-not $serverUri.IsAbsoluteUri -or $serverUri.Scheme -notin @('http', 'https') -or $serverUri.UserInfo -ne '' -or $serverUri.Query -ne '' -or $serverUri.Fragment -ne '') { throw 'Invalid server address' }
} catch {
  Exit-Fail 'Invalid or expired enrollment. Generate a new command in bb.'
}
$HostId = [string]$bootstrap.hostId
$script:HostIdValue = $HostId
$serverUrl = [string]$bootstrap.serverUrl
if ($Start -or $Stop -or $Restart -or $Uninstall) {
  Invoke-ServiceLifecycle $requestedDataDir $HostId $serverUrl
  exit 0
}
...
if ([string]::IsNullOrWhiteSpace($dataDir)) { $dataDir = $env:BB_DATA_DIR }
if ($bootstrap.PSObject.Properties['dataDir'] -and -not [string]::IsNullOrWhiteSpace($bootstrap.dataDir)) {
  if (-not [string]::IsNullOrWhiteSpace($dataDir) -and $dataDir -ne $bootstrap.dataDir) {
    Exit-Fail 'BB_DATA_DIR does not match the enrolled machine data directory.'
  }
  $dataDir = [string]$bootstrap.dataDir
}
...
if (-not $Adopt) {
  $installedHostId = ''
  try { $installedHostId = [IO.File]::ReadAllText((Join-Path $dataDir 'host-id')).Trim() } catch {}
  if ($installedHostId -eq '') {
    try { $installedAuth = ConvertFrom-Json ([IO.File]::ReadAllText((Join-Path $dataDir 'auth.json'))); $installedHostId = [string]$installedAuth.hostId } catch {}
  }
  if ($installedHostId -ne '' -and $installedHostId -ne $HostId) {
    Exit-Fail 'This data directory belongs to another machine. Select a new BB_DATA_DIR or run reconnect on the enrolled computer.'
  }
  if ($bootstrap.PSObject.Properties['reconnect'] -and $bootstrap.reconnect -and $installedHostId -ne $HostId) {
    Exit-Fail 'This machine is not installed in the selected data directory. Run reconnect on the enrolled computer.'
  }
}
...
$requestHeaders = @()
if ($bootstrap.PSObject.Properties['headers']) {
  foreach ($header in $bootstrap.headers.PSObject.Properties) {
    if ($header.Name -match '[\s:]' -or $header.Value -isnot [string] -or [string]$header.Value -match '[\r\n]') {
      Exit-Fail 'Invalid enrollment request headers.'
    }
    $requestHeaders += "$($header.Name): $($header.Value)"
  }
}
if ($installedArtifactDigest -ne '') {
  $requestHeaders += "If-None-Match: `"sha256-$installedArtifactDigest`""
}
$configLines = @($requestHeaders | ForEach-Object { 'header = "' + $_.Replace('\', '\\').Replace('"', '\"') + '"' })
[IO.File]::WriteAllLines($curlConfigFile, [string[]]$configLines, (New-Object Text.UTF8Encoding $false))
try {
  ...
  if ($download.ExitCode -eq 0) { $packageStatus = $download.Output }
} finally {
...
if (($packageStatus -eq '304') -and ($installedArtifactDigest -ne '')) {
  ...
  & {
    $ErrorActionPreference = 'Continue'
    & $nodeExe $npmCli install -g $bbAppAllowScripts --prefix $machineNpmPrefix $packageFile
  }
  ...
  Remove-Item -LiteralPath $packageDir -Recurse -Force -ErrorAction SilentlyContinue
  Exit-Fail "Could not download the server's bb-app package from $packageUrl (HTTP $packageStatus)." @('The Windows host requires the exact server build. Retry after the server artifact is available.')
}
Remove-Item -LiteralPath $packageDir -Recurse -Force -ErrorAction SilentlyContinue
...
$taskName = "bb-host-daemon-$HostId"
$wrapperScript = Join-Path $dataDir "$taskName.ps1"
...
$wrapperLines = @(
  ...
  '& ' + ((@($nodeExe, $bbAppJs, 'host-daemon', '--auto-update', '--host-daemon-port', $selectedHostDaemonPort, '--server-url', $serverUrl) | ForEach-Object { Quote-PowerShell $_ }) -join ' ')
  'exit $LASTEXITCODE'
)
[IO.File]::WriteAllLines($wrapperScript, [string[]]$wrapperLines, (New-Object Text.UTF8Encoding $true))
[IO.File]::WriteAllText((Join-Path $dataDir 'install-machine.ps1'), $script:InstallerSource, (New-Object Text.UTF8Encoding $true))
if ((Get-ListenerProcessId $selectedHostDaemonPort) -ne 0) {
  Invoke-ServiceLifecycle -Directory $dataDir -MachineId $HostId -ServerUrl $serverUrl -StopOnly
}

if (-not $Adopt) {
$bbCli = Join-Path $bbAppNpmPrefix 'node_modules\bb-app\dist\bb.js'
if (-not (Test-Path -LiteralPath $bbCli)) {
  Exit-Fail 'The installed build does not provide the machine enrollment CLI.'
}
$previousDataDir = $env:BB_DATA_DIR
$previousCli = $env:BB_CLI
Remove-Item Env:BB_CLI -ErrorAction SilentlyContinue
$env:BB_DATA_DIR = $dataDir
$env:BB_ENROLLMENT = $bootstrapPayload
try {
  & $nodeExe $bbCli machine enroll --bootstrap-env BB_ENROLLMENT
  if ($LASTEXITCODE -ne 0) { Exit-Fail 'Machine enrollment failed.' }
} finally {
  $env:BB_DATA_DIR = $previousDataDir
  $env:BB_CLI = $previousCli
  Remove-Item Env:BB_ENROLLMENT -ErrorAction SilentlyContinue
  $bootstrapPayload = $null
}
}
$identityPath = Join-Path $dataDir 'host-id'
[IO.File]::WriteAllText($identityPath, $HostId, (New-Object Text.UTF8Encoding $false))
$joinPid = 0
...
if ($env:BB_INSTALL_SKIP_SERVICE -eq '1') {
  if (($joinPid -eq 0) -and (-not (Test-DaemonStatusMatches -Port $selectedHostDaemonPort -ExpectedHostId $HostId -ExpectedServerUrl $serverUrl -RequireConnected $false))) {
    ...
    $joinPid = Start-DetachedDaemonProcess -WrapperScript $wrapperScript -LogPath $daemonLog -WorkingDir $dataDir
...
if ($env:BB_INSTALL_FORCE_RUNKEY -eq '1') {
  ...
  $persistence = Install-ScheduledTaskPersistence -TaskName $taskName -WrapperScript $wrapperScript -DataDir $dataDir -Port $selectedHostDaemonPort -ServerUrl $serverUrl
}
if ($persistence.Kind -eq 'access-denied') {
  ...
  $runResult = Install-RunKeyPersistence -ValueName $taskName -WrapperScript $wrapperScript -DataDir $dataDir -Port $selectedHostDaemonPort -ServerUrl $serverUrl
  ...
  $serviceManager = 'windows-run-key'
} else {
  $serviceDesc = "Scheduled task \$taskName"
  $serviceManager = 'windows-task'
}
$wrapperLines = @($wrapperLines[0], ('$bbServiceManager = ' + (Quote-PowerShell $serviceManager))) + $wrapperLines[1..($wrapperLines.Count - 1)]
[IO.File]::WriteAllLines($wrapperScript, [string[]]$wrapperLines, (New-Object Text.UTF8Encoding $true))
...
Write-Detail ('Manage: & ' + (Quote-PowerShell (Join-Path $dataDir 'install-machine.ps1')) + ' -Restart|-Stop|-Start|-Uninstall -DataDir ' + (Quote-PowerShell $dataDir))
