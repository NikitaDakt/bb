...
describe("TerminalManager", () => {
  ...
  it("resolves powershell.exe by default on win32 without ambient platform detection", async () => {
    ...
    expect(resolvedShell.toLowerCase()).toMatch(/(powershell|pwsh)(\.exe)?$/u);
    expect(harness.adapter.spawned[0]?.args.args).toEqual([
      "-NoLogo",
      "-NoProfile",
      "-NoExit",
...
describe("terminal sweep registration", () => {
  ...
  it("registers late when the pty pid arrives after spawn", async () => {
    const harness = createHarness();
