...
describe("desktop update feed url", () => {
  ...
  it("polls the Windows moving tag instead of the shared release tag", () => {
    expect(createDesktopUpdateFeedUrl("windows")).toBe(
      "https://github.com/get-bb/bb/releases/download/desktop-win-latest/desktop-version-windows.json",
    );
  });
  ...
  it("resolves the version feed url per platform and channel", () => {
    ...
    expect(createDesktopUpdateFeedUrlForChannel("windows", "nightly")).toBe(
      "https://github.com/get-bb/bb/releases/download/desktop-win-nightly/desktop-version-windows.json",
    );
  });
});
...
describe("desktop update support", () => {
  ...
  it("supports the native Windows installer and version feed", () => {
    let consulted = false;
...
it("keeps custom desktop builds on their configured release repository", () => {
  expect(
    createDesktopReleaseInfo("latest", "owner/bb-windows")
      .windowsUpdateReleaseBaseUrl,
  ).toBe(
    "https://github.com/owner/bb-windows/releases/download/desktop-win-latest/",
  );
  expect(() =>
    createDesktopReleaseInfo("latest", "https://example.com"),
